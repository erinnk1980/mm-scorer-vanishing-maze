...
public class VanishingMazeVis {
    ...
    volatile double pendingScore;       // pending part of the score: score from the last target number hit to the current step
    // -----------------------------------------
    String generate(String seedStr) {
      try {
        ...
        numbers = new int[S * S];
        // to make sure each number from 1 to N is present at least once, add them up front
        for (int i = 1; i <= N; ++i)
            numbers[i-1] = i;
        ...
        // generate the starting position of the player (make sure it's not on a special tile or on a 1 tile)
        do {
            pRow = r1.nextInt(S);
            pCol = r1.nextInt(S);
    ...
    void takeAction(char act) {
        ...
        if (act == 'U')      newRow = (newRow + S - 1) % S;
        else if (act == 'D') newRow = (newRow + 1) % S;
        else if (act == 'L') newCol = (newCol + S - 1) % S;
        else if (act == 'R') newCol = (newCol + 1) % S;
        else {
        ...
        // destroy tiles > nextNumber and add them to the pending score (with nextNumber multiplier)
        if (numbers[pRow * S + pCol] > nextNumber) {
    ...
    public class Vis extends JPanel {
        public void paint(Graphics g) {
            ...
            for (int c = 0; c < S; ++c) {
                int n = numbers[r * S + c];
                if (n != 0) {
                    // mark the tiles which are not going to disappear (but also don't bring any score)
            ...
            yText += hText;
            drawString(g2,String.format("%.2f", tilesScore * Math.pow((nextNumber - 1) * 1.0 / N, power)),xText,yText,wText,hText,0);
            yText += hText * 2;
